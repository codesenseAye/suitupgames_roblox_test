...
local ReplicaService = {
	...
	CreatedReplicas = {}, -- {[replicaTokenString]: replica}
...
local function DestroyReplicaAndDescendantsRecursive(replica, not_first_in_stack)
	...
	-- Clear replica entry:
	Replicas[id] = nil
	replica.Destroyed = true
	ReplicaService.CreatedReplicas[replica.Class] = nil
...
function ReplicaService.NewReplica(replica_params) --> [Replica]
	...
	local meta_table = replica_params.Meta or {}
	...
	if type(meta_table) ~= "table" then
		error("[ReplicaService]: replica_params.Meta must be a table")
	end
	...
	ReplicaService.CreatedReplicas[replica.Class] = replica
